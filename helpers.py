...
def get_db():
    """Return the per-request PostgreSQL connection wrapper."""
    from db import get_db as _get_pg_db
    return _get_pg_db()
...
def add_notification(db, user_id, message):
    db.execute('INSERT INTO notifications (user_id, message) VALUES (%s,%s)',
               (user_id, message))
...
def check_and_award_referral_bonus(db, user_id):
    ...
    user = db.execute(
        'SELECT referred_by, referral_bonus_awarded FROM users WHERE id=%s', (user_id,)
    ).fetchone()
    if not user or not user['referred_by'] or user['referral_bonus_awarded']:
        return

    balance_row = db.execute('SELECT balance FROM users WHERE id=%s', (user_id,)).fetchone()
    ...
    referred_user = db.execute('SELECT username FROM users WHERE id=%s', (user_id,)).fetchone()
    ...
    db.execute('UPDATE users SET referral_bonus_awarded=1 WHERE id=%s', (user_id,))
    add_notification(
        db, user['referred_by'],
...
def format_post(row, current_uid, db):
    ...
    if p.get('repost_of_id'):
        orig_row = db.execute('SELECT * FROM posts WHERE id=%s', (p['repost_of_id'],)).fetchone()
    ...
    if p.get('reply_to_id'):
        parent = db.execute(
            'SELECT p.id, u.username FROM posts p JOIN users u ON p.user_id=u.id WHERE p.id=%s',
            (p['reply_to_id'],)
        ).fetchone()
    ...
    if p.get('is_subscriber_only') and p['user_id'] != current_uid:
        ...
        viewer = db.execute('SELECT is_admin FROM users WHERE id=%s', (current_uid,)).fetchone()
...
def format_post_with_poll(row, uid, db):
    ...
    if p and p.get('post_type') == 'poll':
        options = db.execute(
            'SELECT * FROM poll_options WHERE post_id=%s', (p['id'],)
        ).fetchall()
        total_votes = sum(o['votes'] for o in options)
        user_vote = db.execute(
            'SELECT option_id FROM poll_votes WHERE post_id=%s AND user_id=%s',
            (p['id'], uid)
        ).fetchone()
        p['poll'] = {
            'options': [
...
def update_counts(db, user_id):
    ...
    db.execute("""UPDATE users SET
        follower_count  = (SELECT COUNT(*) FROM follows WHERE following_id=%s),
        following_count = (SELECT COUNT(*) FROM follows WHERE follower_id=%s),
        post_count      = (SELECT COUNT(*) FROM posts WHERE user_id=%s AND reply_to_id IS NULL)
        WHERE id=%s""", (user_id, user_id, user_id, user_id))
...
def recalc_post_score(db, post_id):
    ...
    db.execute('UPDATE posts SET score=%s WHERE id=%s', (round(score, 6), post_id))
...
def get_personalized_post_ids(db, uid, limit=20, offset=0):
    ...
    seen = {r[0] for r in db.execute(
        'SELECT post_id FROM post_views WHERE user_id=%s', (uid,)
    ).fetchall()}
    liked = {r[0] for r in db.execute(
        'SELECT post_id FROM post_likes WHERE user_id=%s', (uid,)
    ).fetchall()}
    exclude = seen | liked

    following_ids = [r[0] for r in db.execute(
        'SELECT following_id FROM follows WHERE follower_id=%s', (uid,)
    ).fetchall()]
    ...
    if following_ids:
        ...
        rows = db.execute(
            ...
            f'WHERE l.user_id IN ({ph}) AND p.user_id != %s '
